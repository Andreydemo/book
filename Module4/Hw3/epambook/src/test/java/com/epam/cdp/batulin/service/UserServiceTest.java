...
public class UserServiceTest {
    ...
    @Test
    public void whenAddNoteToUsersTimelineThenNoteIsAdded() {
        User user = new User("username", "name", new Date(1));
        Note note = new Note("noteText");
        when(userRepository.findByUsername(user.getUsername())).thenReturn(user);
        when(userRepository.save(any(User.class))).thenReturn(user);

        User updatedUser = userService.addNoteToUsersTimeline(user, note);
        assertTrue(updatedUser.getTimeline().contains(note));
    }

    @Test
    public void whenAddNoteToNonExistingUsersTimelineThenUserNotFoundExceptionIsThrown() {
        String username = "username";
        User user = new User(username, "name", new Date(1));
        exception.expect(UserNotFoundException.class);
        exception.expectMessage("user with username = " + username + " cannot be found");
        ...
        userService.addNoteToUsersTimeline(user, new Note("text"));
    }
