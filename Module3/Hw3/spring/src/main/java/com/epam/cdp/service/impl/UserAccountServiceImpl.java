...
public class UserAccountServiceImpl implements UserAccountService {
    ...
    public UserAccount refillAccount(long userId, BigDecimal amount) {
        if (amount == null || amount.compareTo(new BigDecimal(BigInteger.ZERO)) < 0) {
            logger.debug("amount is null or less then zero, account cannot be refilled");
            return null;
        }
    ...
    public UserAccount withdraw(long userId, BigDecimal amount) {
        if (amount == null || amount.compareTo(new BigDecimal(BigInteger.ZERO)) < 0) {
            logger.debug("amount is null or less then zero, account cannot be withdrawn");
            return null;
        }
    ...
    public UserAccount create(UserAccount userAccount) {
        if (userAccount == null) {
            logger.debug("user account is null, returning null");
            return null;
        }
