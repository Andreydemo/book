...
public class UserController {
    ...
    public
    @ResponseBody
    UserImpl createUser(@RequestBody UserImpl user) {
        User createdUser = bookingFacade.createUser(user);
        logger.debug("Creating user: " + user);
        return (UserImpl) createdUser;
    }
    ...
    public
    @ResponseBody
    UserImpl updateUser(@RequestBody UserImpl user) {
        User updatedUser = bookingFacade.updateUser(user);
        logger.debug("Updating user: " + user);
        return (UserImpl) updatedUser;
    }
     ...
     *
     * @param userAccount account of the user to update.
     * @return Refilled UserAccount.
     */
    @RequestMapping(value = "/{userId}/balance", method = RequestMethod.PUT)
    public
    @ResponseBody
    UserAccount refillAccount(@PathVariable long userId, @RequestBody UserAccountImpl userAccount) {
        logger.debug("Refilled UserAccount for userId = " + userId + ", amount = " + userAccount.getBalance());
        return bookingFacade.refillAccount(userId, userAccount.getBalance());
    }
}