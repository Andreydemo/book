...
public class EventController {
    ...
    public
    @ResponseBody
    Resource<Event> getEvent(@PathVariable long eventId) {
        Event event = bookingFacade.getEventById(eventId);
        logger.debug("Returning event by id: " + eventId + ", " + event);
        Resource<Event> resource = new Resource(event);
        resource.add(linkTo(methodOn(EventController.class).getEvent(eventId)).withSelfRel());
        return resource;
    }
